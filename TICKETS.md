...
| CMP-012 | Repara Hero y Gallery en mobile (imágenes invisibles en <640px) | in-progress | `CMP-012-fix-mobile-hero-gallery` | 2026-09-07 | El wrapper de la imagen del Hero y las celdas de `.gallery-grid` dependían de que el layout padre (flex-row / grid con filas fijas) les diera tamaño; al cambiar a columna/auto en el breakpoint de 640px, colapsaban a ~0 porque su único contenido era `position: absolute`. Fix: `hero-img-wrap`/`hero-img-frame` con ancho/alto explícitos en mobile, y `.gallery-grid` con `grid-template-rows` fijo en vez de `auto`. Actualiza `02-design-system.md`. |
